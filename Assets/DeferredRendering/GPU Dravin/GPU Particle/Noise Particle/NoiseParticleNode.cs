...
namespace DefferedRender
{
    ...
    public class NoiseParticleNode : GPUDravinBase
    {
        ...
        private void SetOnCompute()
        {
            ...
            compute.SetVector("_BeginSpeed", transform.TransformDirection(noiseData.velocityBegin));
        ...
        {
            //未启动，且不是因为等待关闭时造成的为未启用才退出
            if (!gameObject.activeSelf && waitDesTime < 0) return;
        ...
        private float waitDesTime = -1;

        /// <summary>
        /// 设置该粒子为待删除状态，也就是等待粒子是否时间结束后删除
        /// </summary>
        public void WaitToDestory()
        {
            waitDesTime = Time.deltaTime;
            isAutoPlay = false;
            Common.SustainCoroutine.Instance.AddCoroutine(SustainUpdate);
        }

        //维持粒子更新
        private bool SustainUpdate()
        {
            waitDesTime += Time.deltaTime;
            if (waitDesTime > noiseData.releaseTime + noiseData.liveTime)
            {
                Destroy(gameObject);
                return true;
            }
