...
float3 GetDefferFog (float4 positionCS, float3 positionWS, float3 finalCol) {
	...
	// float noise = Perlin3DFBM(positionWS * 0.3 + float3(_Time.y, 0, _Time.y) * 0.3, 1);
	// noise = saturate(noise);

	// depthRadio += (1.0 - depthRadio) * noise;
...
float3 GetDefferFog(float bufferDepth, float3 positionWS, float3 finalCol){
	...
	//确定在范围中的比例
    bufferDepth = 1 - saturate( (_FogMaxDepth - bufferDepth) / ( _FogMaxDepth - _FogMinDepth ) );
	//根据平方缩减
    float depthRadio = pow(bufferDepth, _FogDepthFallOff);
	...
	return lerp(finalCol, _FogColor.xyz, posYRadio * depthRadio);
}

float GetDefferFogRadio(float bufferDepth, float3 positionWS){
	bufferDepth = Linear01Depth(bufferDepth, _ZBufferParams);
	//确定在范围中的比例
    bufferDepth = 1 - saturate( (_FogMaxDepth - bufferDepth) / ( _FogMaxDepth - _FogMinDepth ) );
	//根据平方缩减
	...
	float noise = Perlin3DFBM(positionWS * 0.3 + float3(_Time.y, 0, _Time.y) * 0.3, 1);
	noise = saturate(noise);
	return depthRadio * posYRadio * noise;
}
