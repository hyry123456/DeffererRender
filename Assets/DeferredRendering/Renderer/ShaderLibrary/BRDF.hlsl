...
float3 DirectBSDF(Surface surface, Light light, float normalDistorion, float power, float width, float scale){
	float3 backLightDir = surface.normal * normalDistorion + light.direction;
	float fLTDot = pow(saturate( dot(surface.viewDirection, -backLightDir)), power ) 
		* light.attenuation * scale;
	float3 transferColor = surface.color * light.color;
	float3 transferCol = fLTDot * transferColor * light.color * (1.0 - width);
	...
	return ((specularTerm + diff) * nl + transferCol) * light.attenuation;
}
