...
float hash1d(float input) {
    const float k = 0.3678794;
    input = input * k + k;
    return -1.0 + 2.0 * frac(16.0 * k * frac(input * input.x));
}
...
float Perlin1DFun(float pos) {
    float i = floor(pos);
    float f = frac(pos);

    float2 g0 = hash1d(i);
    float2 g1 = hash1d(i + 1);

    //获得点乘后的大小
    float v0 = dot(g0, f - 0.0);
    float v1 = dot(g1, f - 1.0);

    float inter0 = Cos_Interpolate(v0, v1, f);
...
float Perlin1DFBM(float pos, int octave) {
    float noise = 0.0;
    float frequency = 1.0;
    float amplitude = 1.0;
    ...
    for (int i = 0; i < octave; i++)
    {
        noise += Perlin1DFun(pos * frequency) * amplitude;
        frequency *= 2.0;
        amplitude *= 0.5;
    }
    return noise;
